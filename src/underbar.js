...
(function() {
  ...
  _.map = function(array, iterator) {
    var results = [];
    _.each(array, function(item){results.push(iterator(item))});
    return results;
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    for(var x=2;x<arguments.length;x++){
      argArr[x]=arguments[x];
    }
    for(var i=0;i<collection.length;i++){
      if(typeof(functionOrKey)== "function"){
        array.push(functionOrKey.apply(collection[i], argArr));
      }else{
  ...
  _.shuffle = function(array) {
    ...
    for (var i=0;i<array.length;i++){
      ...
      var randIndex = Math.random() * (array.length-1);
      while(myArr[randIndex]!==undefined){
        randIndex = Math.random() * (array.length-1);
      }
      myArr[randIndex] = newArr[i];
    }
    return myArr;
